...
def merge_embedding(
    old: Sequence[float] | None, old_count: int, new: Sequence[float]
) -> tuple[list[float], int]:
    """把一个新向量并入族代表均值，返回 ``(新均值, 新计数)``。

    P0-3 口径：``old_count`` 必须是实际参与过均值的向量条数
    （``creatives.embedding_count``），不是全部 variant 数——否则历史
    无向量 variant 会稀释新向量、过度加权最早进族的向量（E0 漂移复现：
    5 variant 仅 3 有向量时族中心偏移 31.83°）。
    """
    if old and len(old) == len(new) and old_count > 0:
        return running_mean(old, new, old_count), old_count + 1
    return list(new), 1


def mean_embeddings(vectors: Sequence[Sequence[float]]) -> list[float] | None:
    """一组向量的算术均值（用于按成员重算族代表向量 / 拆族新族代表）。

    维度不一致的向量被跳过（以第一个非空向量的维度为准）；无可用向量
    返回 None。均值与顺序无关，回填/重算幂等。
    """
    dim = 0
    ...
    if not usable:
        return None
    count = len(usable)
    return [sum(vector[i] for vector in usable) / count for i in range(dim)]


def cosine_matrix(vectors: Sequence[Sequence[float]]) -> list[list[float]]:
    """NumPy 矩阵化两两余弦（E2 召回通道的批量打分用）。

    语义与 :func:`cosine_similarity` 逐对一致（有交叉断言测试锁定）：
    空向量 / 维度与首个非空向量不符 / 零模长 → 与该向量相关的全部为 0.0
    （对角线例外：非零向量与自身恒为 1.0，与标量实现一致）。
    """
    n = len(vectors)
    if n == 0:
        return []
    ...
    for i, vector in enumerate(vectors):
        if vector and len(vector) == dim:
            mat[i] = np.asarray(vector, dtype=np.float64)
    norms = np.linalg.norm(mat, axis=1)
    nonzero = norms > 0.0
    if nonzero.any():
        unit = mat[nonzero] / norms[nonzero, None]
        sims[np.ix_(nonzero, nonzero)] = unit @ unit.T
    # 对角线按标量实现的逐对语义补齐：维度不符（但非零模长）的向量与
    # 自身余弦仍为 1.0（标量实现里同长度必可比）
    for i, vector in enumerate(vectors):
        if vector and any(x != 0.0 for x in vector):
            sims[i, i] = 1.0
    return [[float(sims[i, j]) for j in range(n)] for i in range(n)]
