...
/** POST /dnas — 手动创建 DNA 家族（code 由后端发号）。 */
export function createDna(body: {
  name: string;
  hook_prototype?: string;
  core_mechanic?: string;
  narrative_structure?: string;
}): Promise<DnaFamily> {
  return unwrap(apiClient.post<ApiResponse<DnaFamily>>("/dnas", body));
}

/** POST /dnas/suggest-families — 智能建族：LLM 批量提案进收件箱，返回提案族数。 */
export function suggestFamilies(): Promise<number> {
  // Bootstrap clusters all unassigned creatives and drafts one family per
  // cluster via LLM — synchronous and slow on larger libraries, so allow a
  // long wait like upload (default 60s aborts mid-run with a false failure).
  return unwrap(
    apiClient.post<ApiResponse<number>>("/dnas/suggest-families", null, { timeout: 300000 }),
  );
}

/** POST /dnas/confirm-family — 确认一个智能建族提案（允许改名后确认；
 *  带 existing_dna_code 时并入已有家族，不建族只挂接，name 传 ""）。 */
export function confirmFamily(body: {
  ...
  return unwrap(apiClient.post<ApiResponse<DnaFamily>>("/dnas/confirm-family", body));
}

/** POST /dnas/dismiss-family — 跳过提案（只删建议，不建族）。 */
export function dismissFamily(suggestionId: string): Promise<null> {
  return unwrap(
    apiClient.post<ApiResponse<null>>("/dnas/dismiss-family", {
      suggestion_id: suggestionId,
    }),
  );
}

export function confirmRuleKeyword(suggestionId: string): Promise<null> {
  return unwrap(
    apiClient.post<ApiResponse<null>>("/dnas/rule-keywords/confirm", {
      suggestion_id: suggestionId,
    }),
  );
}

export function dismissRuleKeyword(suggestionId: string): Promise<null> {
  return unwrap(
    apiClient.post<ApiResponse<null>>("/dnas/rule-keywords/dismiss", {
      suggestion_id: suggestionId,
    }),
  );
}
