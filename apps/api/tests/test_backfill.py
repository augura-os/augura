...
def test_market_always_with_market_prefix() -> None:
    assert derive_factors("KS_EN-260101-x.mp4", []) == ["language-market"]
    assert derive_factors("KS_KR-260101-x.mp4", []) == ["language-market"]
...
def test_full_combination_order() -> None:
    factors = derive_factors(
        "KS_EN-260102-58-制作人丙-模拟经营-AI-示例-山林采集-制作人乙-竖.mp4",
