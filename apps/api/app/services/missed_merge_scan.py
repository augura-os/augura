...
4. 向量近邻：representative_embedding 余弦 top-k（k=EMBEDDING_RECALL_TOP_K）
   ——shadow-only：只进候选并集让 LLM 判定，永不接管 attach（E2）

排除：已有裁决（split_rulings / case-rulings）、观察对、同族（同 DNA）、
已归档 creative。判定沿用 merge_judge 3 票防偏；双证据齐（LLM 3/3 +
pHash 对齐 ≥0.90）且 merge_auto_enabled + judge_auto_allowed(merge_pair)
开才自动合并（merge_ops），否则写收件箱建议（reason 带三路证据）。
...
from app.services.embedding import EMBEDDING_RECALL_TOP_K
...
@dataclass
class RecallPair:
    """一对召回的候选 creative 及四路证据分（None = 该路未命中/未测）。"""
    ...
    def evidence(self) -> str:
        ...
        if self.embedding_score is not None:
            parts.append(f"向量 {self.embedding_score:.2f}")
        return " / ".join(parts)
...
def _embedding_topk_recall(
    creatives: Sequence[Creative],
    pairs: Sequence[tuple[Creative, Creative]],
    excluded: set[frozenset],
) -> list[tuple[Creative, Creative, float]]:
    """向量近邻通道（E2，shadow-only）：每个 creative 按 representative_embedding
    余弦取 top-k（k=EMBEDDING_RECALL_TOP_K）近邻，命中对 → (left, right, 余弦分)。

    不用全局阈值（E0 实测：bge 分布压缩在 [0.6,1.0]，阈值跨模型不可移植，
    能排序就别用阈值）。只认 ``pairs`` 里的候选（scoped 模式只扫 anchor 相关
    对）且尊重与其他通道一致的排除（已裁决/观察对/同族/case-rulings）。
    """
    embedded = [c for c in creatives if c.representative_embedding]
    if len(embedded) < 2:
        return []
    matrix = cosine_matrix([c.representative_embedding for c in embedded])
    pair_by_key = {frozenset((left.id, right.id)): (left, right) for left, right in pairs}
    hits: dict[frozenset, float] = {}
    for i, left in enumerate(embedded):
        row = matrix[i]
        ranked = sorted(
            (j for j in range(len(embedded)) if j != i and row[j] > 0.0),
            key=lambda j: row[j],
            reverse=True,
        ...
        for j in ranked:
            ...
            if key in excluded or key not in pair_by_key:
                continue
            if find_prior_ruling(left.name, right.name) is not None:
                continue  # case-rulings 既定拆分裁决
            hits[key] = max(row[j], hits.get(key, 0.0))
    return [(*pair_by_key[key], score) for key, score in hits.items()]
...
def recall_pairs(
    ...
    *,
    ...
    """四路召回并集。``creative_id`` 给了就只扫它 vs 全部（O(N)，上传/
    ...
    # 第 4 路：向量近邻 top-k 并集（shadow-only：只进候选池让 LLM 判定）
    for left, right, score in _embedding_topk_recall(creatives, pairs, excluded):
        key = frozenset((left.id, right.id))
        pair = recalled.get(key)
        if pair is None:
            pair = RecallPair(left=left, right=right)
            recalled[key] = pair
        pair.embedding_score = score
...
def scan_missed_merges(
    ...
    *,
    ...
    auto_merge = (
        not dry_run
        # 总闸 AND 类别闸（merge_pair）：刹车踩下后本扫描不再自动执行
        and judge_auto_allowed(db, "merge_pair")
