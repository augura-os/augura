...
from app.services import family_bootstrap, markets
...
def auto_brake_items(db: Session) -> list[ReviewItem]:
    """刹车通知（judge_calibration 自动降级/恢复的留痕，纯告知无处置）。"""
    return [
        ReviewItem(
            kind="auto_brake",
            title=(
        ...
        for row in db.scalars(
            select(JudgeSuggestion)
            .where(JudgeSuggestion.kind == "auto_brake")
            .order_by(JudgeSuggestion.created_at.desc())
        ).all()
    ]
...
def rule_keyword_items(db: Session) -> list[ReviewItem]:
    """规则词建议（rule_feedback 从人工改判挖出的候选词，确认才落 settings）。

    reason 列存 JSON 负载（不改表结构，与 family_bootstrap 同模式）；
    负载坏了的条目静默跳过（不挡其他建议浮出）。
    """
    rows = db.scalars(
        select(JudgeSuggestion)
        .where(JudgeSuggestion.kind == "rule_keyword")
        .order_by(JudgeSuggestion.created_at)
    ).all()
    items: list[ReviewItem] = []
    for row in rows:
        ...
        proposal = RuleKeywordProposal(
            suggestion_id=row.id,
            target=str(payload.get("target") or "hook"),
            word=str(payload.get("word") or row.verdict),
            score=float(payload.get("score") or 0.0),
            same_pair_rate=float(evidence.get("same_pair_rate") or 0.0),
            cross_pair_rate=float(evidence.get("cross_pair_rate") or 0.0),
            support=int(evidence.get("support") or row.votes or 0),
            learned_at=str(payload.get("learned_at") or ""),
        )
        label = _RULE_TARGET_LABELS.get(proposal.target, proposal.target)
        items.append(
            ReviewItem(
                kind="rule_keyword",
...
def family_bootstrap_hint(db: Session) -> FamilyBootstrapHint:
    """稳态增量扩族提示：散点 ≥ SUGGEST_THRESHOLD（约一批）且无未处理提案时
    suggest=True——已有提案浮在收件箱时不催，避免重复打扰。"""
    unassigned = family_bootstrap.count_unassigned(db)
    pending = int(
        db.scalar(
            select(func.count(JudgeSuggestion.id)).where(
                JudgeSuggestion.kind == family_bootstrap.KIND
            )
        )
        or 0
    )
    return FamilyBootstrapHint(
        unassigned=unassigned,
        pending_proposals=pending,
        suggest=unassigned >= family_bootstrap.SUGGEST_THRESHOLD and pending == 0,
    )


def family_bootstrap_items(db: Session) -> list[ReviewItem]:
    """智能建族提案（services/family_bootstrap 产出，人工逐族确认/跳过）。

    reason 列存 JSON 负载（不改表结构）；成员只列仍是散点的 creative
    （提案生成后可能已被其他流程归族），成员空了的提案不再浮出。
    """
    rows = list(
        db.scalars(
            select(JudgeSuggestion)
            .where(JudgeSuggestion.kind == "family_bootstrap")
            .order_by(JudgeSuggestion.created_at)
        ).all()
    )
    creatives = {
        c.id: c
        for c in db.scalars(select(Creative).where(Creative.dna_id.is_(None))).all()
    }
    items: list[ReviewItem] = []
    for row in rows:
        try:
        ...
        if not isinstance(member_ids, list):
            continue
        members = [
            FamilyMember(id=mid, name=creatives[mid].name)
            for mid in member_ids
            if isinstance(mid, str) and mid in creatives
        ]
        if not members:
            continue
        name = str(payload.get("name") or row.verdict)
        existing_code = str(payload.get("existing_dna_code") or "").strip() or None
        title = f"并入 {existing_code} · {name}" if existing_code else name
        items.append(
            ReviewItem(
                ...
                family=FamilyProposal(
                    suggestion_id=row.id,
                    name=name,
                    core_mechanic=str(payload.get("core_mechanic") or ""),
                    hook_prototype=str(payload.get("hook_prototype") or ""),
                    narrative_structure=str(payload.get("narrative_structure") or ""),
                    keywords=[
                        str(word) for word in payload.get("keywords") or []
                        if isinstance(word, str) and word.strip()
                    ],
                    members=members,
                    existing_dna_code=existing_code,
                ),
            )
        )
    return items
