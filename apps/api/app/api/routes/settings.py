...
def get_settings_info(db: DbDep, settings: SettingsDep) -> Envelope[SettingsInfo]:
    ...
    return ok(
        SettingsInfo(
            ...
            embedding_backend=resolve_embedding_backend(db),
            embedding_model_active=repo.get(EMBEDDING_MODEL_ACTIVE_SETTING) or "",
...
def update_settings(
    ...
    if payload.embedding_backend is not None:
        if payload.embedding_backend not in EMBEDDING_BACKENDS:
            raise ApiError(
                400,
