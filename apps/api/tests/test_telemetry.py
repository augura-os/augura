...
class TestAnonymize:
    ...
    def test_scrub_slugs_structure_matching_any_language(self) -> None:
        """结构匹配（≥3 段连字符，任意文字系统）：CJK 命名同样脱敏。"""
        from telemetry.anonymize import scrub_slugs

        # CJK 三连段被哈希（此前只认 ASCII kebab 的缺口）
        assert scrub_slugs("山林采集-制作人丙-第三期", salt="s").startswith("h_")
        # 混合文本：两个命名串都哈希、中文散字保留
        mixed = scrub_slugs("素材 jungle-farm-tycoon 与 丛林-买枪-丙 对比", salt="s")
        assert "jungle-farm-tycoon" not in mixed and "丛林-买枪-丙" not in mixed
        assert "素材" in mixed and "对比" in mixed

    def test_scrub_slugs_keeps_non_triple_hyphen_text(self) -> None:
        """无三连连字符的文本原样保留（两段中文命名、中文理由）。"""
        from telemetry.anonymize import scrub_slugs

        assert scrub_slugs("山林采集-制作人丙", salt="s") == "山林采集-制作人丙"
        reason = "改 Hook：山道救援 → 松岩UE（成本差 -11.06）"
        assert scrub_slugs(reason, salt="s") == reason

    def test_scrub_slugs_dates_hashed_as_before(self) -> None:
        """既有行为锚定：日期串三连结构历史上就被哈希，本次不改。"""
        from telemetry.anonymize import scrub_slugs

        assert scrub_slugs("2026-09-15", salt="s").startswith("h_")
