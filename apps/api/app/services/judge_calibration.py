...
由 ``scripts/judge_calibration``（薄壳 CLI，print 只在那层）与
``scripts/judge_candidates``、``services/consolidation.maybe_consolidate``
（周期巩固随行，自动刹车）调用；收件箱 GET /review/queue 用 judge_stats
做只读展示、用 auto_brake 建议做刹车通知。
"""
...
# 10 样本错 3 个正好 30% 卡在线上——样本越小改判率估计越脆，
# 随机抖动就能踩刹/松刹；30 样本时边界噪声大幅收敛（要错 10 个才触发）
MIN_SAMPLE = 30

MASTER_KEY = "judge_auto_enabled"
KIND_KEY_TEMPLATE = "judge_auto_enabled:{kind}"

# 刹车通知在 judge_suggestions / 收件箱里的 kind（纯告知，无需处置）
AUTO_BRAKE_KIND = "auto_brake"
...
@dataclass
class BrakeEvent:
    """一次刹车动作（降级或恢复），只在开关状态翻转时产生。"""

    kind: str  # "master" = 总闸，其余为类别/动态桶名
    gate_key: str
    action: str  # "downgraded" | "restored"
    auto_total: int
    override_rate: float
...
class CalibrationResult:
    """run_calibration 的结构化结果（print 留给 CLI 包装层）。"""
...
def calibration_by_kind(db: Session) -> dict[str, KindCalibration]:
    ...
    auto_logs = list(
        db.scalars(
            ...
            .where(
                EditLog.new_value.like("auto:%"),
                # auto 判定的落点都是 creative（judge_pipeline/merge_ops/
                # dnas 路由）；排除 system（周期巩固扫描）与 judge（刹车
                # 留痕）等非判定噪音——它们也带 auto: 前缀但不是判定
...
def _record_brake_trace(db: Session, event: BrakeEvent) -> None:
    """刹车动作双留痕：edit_logs（auto: 前缀）+ 收件箱 auto_brake 通知。

    通知按闸门键 upsert（同一闸门反复踩/松只更新同一条），edit_logs
    只在状态翻转时写——调用方保证只在翻转时产生 BrakeEvent，故幂等。
    """
    rate = f"{event.override_rate:.1%}"
    sample = f"{event.auto_total}/{MIN_SAMPLE}"
    if event.action == "downgraded":
        text = f"{event.gate_key} 降级为 false（改判率 {rate}，样本 {sample}）"
        title = "自动判定刹车已生效"
    else:
        text = f"{event.gate_key} 恢复为 true（改判率回落至 {rate}，样本 {sample}）"
        title = "自动判定已恢复"
    EditLogRepository(db).record(
        entity_type="judge",
        entity_id=event.gate_key,
        action="auto_brake",
        field=event.action,
        new_value=f"auto: {text}",
    )
    upsert_suggestion(
        db,
        kind=AUTO_BRAKE_KIND,
        left_id=event.gate_key,
        right_id=None,
        verdict=event.action,
        votes=event.auto_total,
        reason=f"{title}：{text}",
    )


def run_calibration(db: Session) -> CalibrationResult:
    """统计改判率并按需升降级（总闸按整体、类别闸按类别）。

    服务形态：无 print（CLI 包装层负责输出），返回结构化结果；
    不 commit，事务归调用方（consolidation 随行时由管线末尾统一提交）。
    """
    ...
    if total >= MIN_SAMPLE and rate > OVERRIDE_RATE_LIMIT and current != "false":
        ...
        events.append(
            BrakeEvent("master", MASTER_KEY, "downgraded", total, rate)
        )
    elif total >= MIN_SAMPLE and rate <= OVERRIDE_RATE_LIMIT and current == "false":
        _set_gate(db, MASTER_KEY, "true")
        events.append(
            BrakeEvent("master", MASTER_KEY, "restored", total, rate)
        )
    ...
    for kind, bucket in stats.items():
        ...
        if bucket.override_rate > OVERRIDE_RATE_LIMIT and kind_current != "false":
            ...
            events.append(
                BrakeEvent(kind, key, "downgraded",
                           bucket.auto_total, bucket.override_rate)
            )
        elif bucket.override_rate <= OVERRIDE_RATE_LIMIT and kind_current == "false":
            _set_gate(db, key, "true")
            events.append(
                BrakeEvent(kind, key, "restored",
                           bucket.auto_total, bucket.override_rate)
            )

    for event in events:
        _record_brake_trace(db, event)
