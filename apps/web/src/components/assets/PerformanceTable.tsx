...
import { metricLabel, useMetricConfig } from "../../hooks/useMetricConfig";
...
export function PerformanceSummary({ rows }: { rows: PerformanceRecord[] }) {
  // 订阅语言切换：metricLabel 走模块级 translate，靠这里触发重渲染
  useT();
      ...
      <table className="w-full text-xs text-neutral-800">
        <thead>
          <tr className="border-b border-[#e5e5e5] text-left text-neutral-500">
            <th className="py-1 pr-2 font-medium">{t("assets.perf.date")}</th>
            {showName ? <th className="py-1 pr-2 font-medium">{t("assets.perf.asset")}</th> : null}
