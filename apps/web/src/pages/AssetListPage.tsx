    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-5xl space-y-6 p-8">
        <header className="flex items-end justify-between gap-4">
          <div>
            <h1 className="text-lg font-semibold text-neutral-900">{t("assets.list.title")}</h1>
            <p className="mt-1 text-sm text-neutral-500">
              {t("assets.list.subtitle")}
            </p>
          </div>
        ...
        <div className="flex items-center gap-1.5">
          {(
            [
              ["all", t("assets.list.filterAll")],
              ["active", t("assets.list.filterActive")],
              ["watch", t("assets.list.filterWatch")],
              ["archived", t("assets.list.filterArchived")],
            ] as const
          ).map(([key, label]) => (
            <button
              key={key}
        ...
        <div className="rounded-lg border border-[#e5e5e5] bg-white">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-[72px]">{t("assets.list.colPreview")}</TableHead>
                <TableHead>{t("assets.list.colFilename")}</TableHead>
                <TableHead className="w-[180px]">{t("assets.list.colUploaded")}</TableHead>
                <TableHead className="w-[130px]">{t("assets.list.colAiStatus")}</TableHead>
                <TableHead>{t("assets.list.colCreative")}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading
                  ...
                  <TableCell>
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      {asset.confidence !== null && asset.confidence < 0.7 ? (
                        <Badge
                          variant="warning"
                          title={t("assets.list.confidenceTitle").replace(
                            "{pct}",
                            (asset.confidence * 100).toFixed(0),
                          )}
                        >
                          {t("assets.list.needsReview")}
                        </Badge>
                      ) : null}
                    </span>
                  </TableCell>
              ...
              {!isLoading && filtered && filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="py-10 text-center text-sm text-neutral-400">
                    {search ? t("assets.list.emptySearch") : t("assets.list.empty")}
                  </TableCell>
                </TableRow>
              ) : null}
            </TableBody>
        ...
        {isFetching && !isLoading ? (
          <p className="text-xs text-neutral-400">{t("assets.list.refreshing")}</p>
        ) : null}
      </div>
    </div>
  );
