      ...
      <div className="relative flex-1">
        {isLoading ? (
          ...
          <div className="flex h-full items-center justify-center">
            <p className="text-sm text-red-600">
              {error instanceof Error ? error.message : t("graph.page.loadFailed")}
            </p>
          </div>
        ) : !data || data.nodes.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-2">
            <p className="text-sm text-neutral-500">{t("graph.page.empty")}</p>
            <Link
              to="/upload"
              className="text-sm font-medium text-neutral-900 underline underline-offset-2 hover:text-neutral-600"
            >
              {t("graph.page.uploadFirst")}
            </Link>
          </div>
        ) : (
          <ReactFlow
        ...
        <div className="pointer-events-none absolute left-4 top-4 flex flex-col items-start gap-2">
          ...
          <div className="pointer-events-auto mt-2 flex items-center gap-1.5 border-t border-[#f0f0f0] pt-2">
            <span className="text-[11px] text-neutral-400">{t("graph.page.density")}</span>
            {(["full", "compact"] as const).map((option) => (
              ...
              >
                {option === "full" ? t("graph.page.full") : t("graph.page.compact")}
              </button>
            ))}
            {autoCompact && densityOverride === null ? (
              <span className="text-[10px] text-neutral-400">{t("graph.page.autoCompact")}</span>
            ) : null}
