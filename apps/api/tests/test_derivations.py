...
class TestInferFactor:
    def test_market_prefix_wins(self) -> None:
        source = _vi("KS_EN-260101-a-竖.mp4", {"language-market", "aspect-ratio"})
        target = _vi("KS_KR-260101-a-竖.mp4", {"language-market"})
...
def _seed_creative_two_variants(db: Session) -> tuple[Creative, list[CreativeVariant]]:
    ...
    for index, suffix in enumerate(("V1", "V2")):
        asset = CreativeAsset(
            id=str(uuid.uuid4()),
            filename=f"KS_EN-260101-58-测试素材名称很长的素材{suffix}-竖.mp4",
            file_type="video",
        ...
        db.add(
            Performance(
                id=str(uuid.uuid4()),
                creative_name=f"ks_en-260101-58-测试素材名称很长的素材{suffix}",
                date=date(2026, 7, 1),
                spend=100.0 * (index + 1),
                installs=10,
