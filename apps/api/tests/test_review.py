...
class TestMarketKey:
    def test_strips_market_prefix(self) -> None:
        pt = _market_key("KS_EN-260104-58-制作人甲-反复挑战重试Ai片头V1-竖.mp4")
        es = _market_key("KS_KR-260104-58-制作人甲-反复挑战重试Ai片头V1-竖.mp4")
        assert pt == es
...
class TestShortLabel:
    def test_market_and_tail(self) -> None:
        label = short_label("KS_EN-260104-58-制作人甲-模拟经营-AI-制作人丁-反复挑战重试Ai片头V1-竖.mp4")
        # 市场码口径：KS_EN → 后缀 EN → 别名归一 US
        assert label == "US …反复挑战重试Ai片头V1-竖"

    def test_es_market(self) -> None:
        label = short_label("KS_KR-260101-58-制作人己-模拟经营-AI前贴_抓马-制作人乙-苛刻掌柜.mp4")
        assert label == "KR …制作人乙-苛刻掌柜"
    ...
    def test_short_tail_expands_to_three_segments(self) -> None:
        assert short_label("KS_KR-260603-58-a-田园物语Ai片头1-竖.mp4") == "KR …田园物语Ai片头1-竖"
...
class TestMergeCandidates:
    ...
    def test_market_pair_same_creative_not_flagged(self, db_session: Session) -> None:
        ...
        for market in ("KS_EN", "KS_KR"):
            asset = CreativeAsset(
                id=str(uuid.uuid4()),
                filename=f"{market}-260104-58-制作人甲-反复挑战重试Ai片头V1-竖.mp4",
...
class TestDerivationReviewItems:
    ...
    def _seed_derivation(
        ...
        for tag in ("V1", "V2"):
            asset = CreativeAsset(
                id=str(uuid.uuid4()),
                filename=f"KS_EN-260101-58-制作人甲-测试素材{tag}-竖.mp4",
                file_type="video",
                storage_key=f"test/{uuid.uuid4()}",
            )
            db.add(asset)
