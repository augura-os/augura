...
JUDGE_TEMPERATURE_SETTING = "judge_temperature"

# 判定采样温度（0–1）：自洽投票（vote_json 3 票）依赖真实随机性，
# 服务商默认 temperature=0 时三票退化为同一票，置信度信号变虚
...
def _resolve_temperature(db_value: str | None) -> float:
    """判定温度解析：缺失/非法/超范围（0–1）一律回落默认 0.7。"""
    if db_value is None:
    ...
    try:
        ...
        return DEFAULT_JUDGE_TEMPERATURE
    return value if 0.0 <= value <= 1.0 else DEFAULT_JUDGE_TEMPERATURE
...
def resolve_ai_config(db: Session, settings: Settings) -> AIConfig:
    ...
    return AIConfig(
        ...
        judge_temperature=_resolve_temperature(repo.get(JUDGE_TEMPERATURE_SETTING)),
    )
