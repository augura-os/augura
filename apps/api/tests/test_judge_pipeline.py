...
class TestScopedMergeJudgements:
    def test_only_pairs_involving_scoped_creative(
        ...
        tail_a = "260104-58-制作人甲-反复挑战重试Ai片头V1-竖.mp4"
        tail_b = "260105-58-制作人乙-苛刻掌柜催工钱V2-竖.mp4"
...
class TestAutoMergeExecute:
    ...
    def _setup_pair(self, db: Session) -> tuple[Creative, Creative]:
        tail = "260104-58-制作人甲-同一主题自动合并-竖.mp4"
...
class TestPostAnalysisGateWiring:
    """P0-1 回归：run_post_analysis 的自动路径必须读类别闸
    （judge_auto_allowed = 总闸 AND 类别闸）。此前只读总闸，
    类别刹车踩下后自动归族/自动合并照样执行，刹车形同虚设。"""

    def _spy_runners(self, monkeypatch) -> dict[str, bool]:  # noqa: ANN001
        captured: dict[str, bool] = {}

        def _dna(db, config, *, dry_run, auto_enabled, creative_id=None):  # noqa: ANN001, ANN002, ANN202
            captured["dna_assign"] = auto_enabled

        def _merge(db, config, *, dry_run, auto_enabled, settings=None, creative_id=None):  # noqa: ANN001, ANN002, ANN202
            captured["merge_pair"] = auto_enabled

        monkeypatch.setattr(judge_pipeline, "run_dna_assignments", _dna)
        monkeypatch.setattr(judge_pipeline, "run_merge_judgements", _merge)
        return captured

    def test_category_gate_blocks_only_that_kind(
        self, db_session: Session, monkeypatch
    ) -> None:
        creative = _creative_with_analysis(
            db_session, name="c-gate", filename="KS_EN-gate.mp4"
        )
        captured = self._spy_runners(monkeypatch)
        SettingsRepository(db_session).set("judge_auto_enabled:merge_pair", "false")
        db_session.flush()
        judge_pipeline.run_post_analysis(db_session, _fake_config(), creative.id)
        assert captured == {"dna_assign": True, "merge_pair": False}

    def test_master_gate_blocks_every_kind(
        self, db_session: Session, monkeypatch
    ) -> None:
        creative = _creative_with_analysis(
            db_session, name="c-gate2", filename="KS_EN-gate2.mp4"
        )
        captured = self._spy_runners(monkeypatch)
        SettingsRepository(db_session).set("judge_auto_enabled", "false")
        db_session.flush()
        judge_pipeline.run_post_analysis(db_session, _fake_config(), creative.id)
        assert captured == {"dna_assign": False, "merge_pair": False}
